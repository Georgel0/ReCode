...
export default function RegexGenerator({ onLoadData }) {
  ...
  const parseAIResponse = (rawOutput) => {
    try {
      ...
      return { pattern: rawOutput, explanation: "Generated by AI." };
    }
  };
  ...
  useEffect(() => {
    if (onLoadData) {
      ...
      const { pattern, explanation } = parseAIResponse(loadedCode);
      setOutputCode(pattern);
      setExplanation(explanation);
    }
  }, [onLoadData]);

  // FIXED: Real-time Regex Testing Logic
  useEffect(() => {
  ...
  const handleGenerate = async () => {
    ...
    try {
      ...
      if (result && result.convertedCode) {
        const { pattern, explanation } = parseAIResponse(result.convertedCode);
  ...
  const renderHighlightedText = () => {
    ...
    try {
        ...
        // split with capture group keeps the matches in the array
