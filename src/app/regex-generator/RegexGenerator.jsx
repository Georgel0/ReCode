'use client';

import React, { useRef } from 'react';
import { ModuleHeader, EmptyState } from '@/components/layout';
import { CopyButton } from '@/components/ui';
import { useRegexGenerator, CHEATSHEET } from './useRegexGenerator';
import './regexGenerator.css';

const TestCaseItem = ({ test, result, outputCode, flags, updateTestCase, removeTestCase }) => {
  const highlightLayerRef = useRef(null);

  const handleScroll = (e) => {
    if (highlightLayerRef.current) {
      highlightLayerRef.current.scrollLeft = e.target.scrollLeft;
    }
  };

  const renderHighlightedText = (text) => {
    if (!outputCode || !text) return text;
    try {
      const highlightFlags = `g${flags.i ? 'i' : ''}`;
      const regex = new RegExp(outputCode, highlightFlags);
      let lastIndex = 0;
      const elements = [];
      let match;
      regex.lastIndex = 0;

      let safeGuard = 0;

      while ((match = regex.exec(text)) !== null && safeGuard < 1000) {
        if (match[0].length === 0) regex.lastIndex++;

        if (match.index > lastIndex) {
          elements.push(text.substring(lastIndex, match.index));
        }
        elements.push(
          <span key={`${match.index}-match`} className="rg-match-highlight">
            {match[0]}
          </span>
        );
        lastIndex = regex.lastIndex;
        safeGuard++;
      }
      if (lastIndex < text.length) elements.push(text.substring(lastIndex));
      return elements.length > 0 ? elements : text;
    } catch (e) {
      return text;
    }
  };

  const isSuccess = result?.isMatch === test.shouldMatch;

  return (
    <div className="rg-test-case-row">
      <button
        className={`rg-expectation-toggle ${test.shouldMatch ? 'rg-expect-match' : 'rg-expect-fail'}`}
        onClick={() => updateTestCase(test.id, 'shouldMatch', !test.shouldMatch)}
        aria-label={`Toggle expectation. Currently ${test.shouldMatch ? 'expecting match' : 'expecting fail'}`}
      >
        <i className={`fa-solid ${test.shouldMatch ? 'fa-check' : 'fa-ban'}`}></i>
      </button>

      <div className="rg-test-input-wrapper">
        <input
          type="text"
          value={test.text}
          onChange={(e) => updateTestCase(test.id, 'text', e.target.value)}
          onScroll={handleScroll}
          placeholder="Test string..."
          className="rg-test-input-field"
          aria-label="Test case input"
        />
        <div className="rg-test-input-highlight-layer" ref={highlightLayerRef} aria-hidden="true">
          {renderHighlightedText(test.text)}
        </div>
      </div>

      <div className={`rg-result-indicator ${isSuccess ? 'rg-success' : 'rg-fail'}`} title={isSuccess ? "Passed" : "Failed"}>
        {result?.error ? <i className="fa-solid fa-triangle-exclamation" title="Invalid Regex"></i> : <i className={`fa-solid ${isSuccess ? 'fa-check' : 'fa-xmark'}`}></i>}
      </div>

      <button className="rg-remove-btn" onClick={() => removeTestCase(test.id)} aria-label="Remove test case">
        <i className="fa-solid fa-trash"></i>
      </button>
    </div>
  );
};

export default function RegexGenerator() {
  const {
    input, setInput, refineMode, setRefineMode, flavor, setFlavor, flags, setFlags,
    outputCode, setOutputCode, summary, breakdown, testCases, loading, matchResults,
    showCheatsheet, setShowCheatsheet, showTestInfo, setShowTestInfo, lastResult,
    handleGenerate, addTestCase, removeTestCase, updateTestCase, fullString
  } = useRegexGenerator();

  return (
    <div className="rg-module-container">
      <ModuleHeader
        title="Regex Generator"
        description="Describe, Refine, and Battle-Test your Regular Expressions."
        resultData={lastResult}
      />

      {showCheatsheet && (
        <div className="rg-modal-overlay" onClick={() => setShowCheatsheet(false)}>
          <div className="rg-modal-content" role="dialog" aria-modal="true" aria-labelledby="cheatsheet-title" onClick={e => e.stopPropagation()}>
            <h2 id="cheatsheet-title">Regex Cheatsheet</h2>
            <div className="rg-ext-grid">
              {Object.entries(CHEATSHEET).map(([category, items]) => (
                <div key={category} className="rg-cheatsheet-category">
                  <h4 className="rg-cheatsheet-title">{category}</h4>
                  {items.map(item => (
                    <div key={item.token} className="rg-cheatsheet-row">
                      <code>{item.token}</code>
                      <span className="rg-cheatsheet-desc">{item.desc}</span>
                    </div>
                  ))}
                </div>
              ))}
            </div>
            <button className="secondary-button rg-modal-close-btn" onClick={() => setShowCheatsheet(false)}>Close</button>
          </div>
        </div>
      )}

      {showTestInfo && (
        <div className="rg-modal-overlay" onClick={() => setShowTestInfo(false)}>
          <div className="rg-modal-content" role="dialog" aria-modal="true" aria-labelledby="test-info-title" onClick={e => e.stopPropagation()}>
            <h3 id="test-info-title">Test Cases Info</h3>
            <div className="rg-info-grid">
              <strong><i className="fa-solid fa-toggle-on"></i> Set Your Goal</strong>
              <p>Toggle to define expectations: Match (<i className="fa-solid fa-check"></i>) or Ban (<i className="fa-solid fa-ban"></i>).</p>
              <strong><i className="fa-solid fa-keyboard"></i> Input Test Strings</strong>
              <p>Type example text in the center. Matches are highlighted in real-time.</p>
              <strong><i className="fa-solid fa-square-poll-vertical"></i> Verdict</strong>
              <p>Success (<i className="fa-solid fa-check"></i>) or Fail (<i className="fa-solid fa-xmark"></i>).</p>
            </div>
            <button className="secondary-button rg-modal-close-btn" onClick={() => setShowTestInfo(false)}>Close</button>
          </div>
        </div>
      )}

      <div className="rg-converter-grid">
        <div className="rg-panel">
          <div className="rg-panel-header">
            <h3>{refineMode ? 'Refine Pattern' : 'Pattern Description'}</h3>
            <button className="secondary-button" onClick={() => setShowCheatsheet(true)}>
              <i className="fa-solid fa-book"></i> Cheatsheet
            </button>
          </div>

          <div className="rg-controls-group">
            <div className="rg-control-field">
              <label className="rg-label-text">Target Flavor</label>
              <select className="rg-select-input" value={flavor} onChange={(e) => setFlavor(e.target.value)}>
                <option value="JavaScript">JavaScript (Standard)</option>
                <option value="Python">Python</option>
                <option value="PCRE">PCRE (PHP/Go)</option>
              </select>
              {flavor !== 'JavaScript' && (
                <span className="rg-flavor-warning">
                  <i className="fa-solid fa-circle-exclamation"></i> Live tests use JS engine
                </span>
              )}
            </div>

            <div className="rg-control-field">
              <label className="rg-label-text">Flags</label>
              <div className="rg-flag-group">
                {['g', 'i', 'm', 's'].map(flag => (
                  <label key={flag} className="rg-custom-check">
                    <input
                      type="checkbox"
                      checked={flags[flag]}
                      onChange={() => setFlags({ ...flags, [flag]: !flags[flag] })}
                    />
                    <span className="rg-checkbox-box"><i className="fas fa-check"></i></span>
                    <span className="rg-flag-text">{flag}</span>
                  </label>
                ))}
              </div>
            </div>
          </div>

          <textarea
            className="rg-textarea"
            value={input}
            onChange={(e) => setInput(e.target.value)}
            placeholder={refineMode ? "Refine the current regex..." : "Describe what you want to match..."}
            spellCheck="false"
          />

          <div className="rg-action-row">
            {refineMode && (
              <button className="secondary-button" onClick={() => { setRefineMode(false); setInput(''); setOutputCode(''); }}>
                <i className="fa-solid fa-rotate-left"></i> Start Over
              </button>
            )}
            <button className="primary-button rg-action-row-right" onClick={handleGenerate} disabled={loading || !input.trim()}>
              <i className={`fa-solid ${loading ? 'fa-spinner fa-spin' : refineMode ? 'fa-sliders' : 'fa-wand-magic-sparkles'}`}></i>
              {loading ? 'Processing...' : refineMode ? 'Refine Regex' : 'Generate Regex'}
            </button>
          </div>

          {outputCode && (<div className="rg-test-bench">
            <div className="rg-bench-header">
              <div className="rg-selector-name">
                <i className="fa-solid fa-vial"></i>
                <h4>Test Cases</h4>
                <button className="rg-info-trigger" onClick={() => setShowTestInfo(true)} aria-label="More information about test cases">
                  <i className="fas fa-circle-info"></i>
                </button>
              </div>
              <button className="rg-icon-btn" onClick={addTestCase} aria-label="Add Test Case" title="Add Test Case">
                <i className="fa-solid fa-plus"></i>
              </button>
            </div>

            <div className="rg-test-cases-list">
              {testCases.map((test) => (
                <TestCaseItem
                  key={test.id}
                  test={test}
                  result={matchResults[test.id]}
                  outputCode={outputCode}
                  flags={flags}
                  updateTestCase={updateTestCase}
                  removeTestCase={removeTestCase}
                />
              ))}
            </div>
          </div>)}
        </div>

        <div className="rg-panel">
          <div className="rg-panel-header">
            <h3>Result Blueprint</h3>
          </div>

          {outputCode ? (
            <>
              <div className="rg-output-box">
                <div className="rg-code-display">
                  <span className="rg-regex-slash">/</span>
                  <input type="text" value={outputCode} readOnly className="rg-regex-input-read" />
                  <span className="rg-regex-slash">/</span>
                  <span className="rg-regex-flags">
                    {Object.keys(flags).filter(k => flags[k]).join('')}
                  </span>
                  <CopyButton className="copy-btn" iconOnly={true} codeToCopy={fullString} />
                </div>
              </div>

              <div className="rg-ai-summary">
                <strong><i className="fa-solid fa-circle-info"></i> {summary}</strong>
                {breakdown.length > 0 && (
                  <div className="rg-token-grid">
                    {breakdown.map((item, idx) => (
                      <div key={idx} className="rg-token-row">
                        <span className="rg-token-badge">{item.token}</span>
                        <span className="rg-token-desc">{item.description}</span>
                      </div>
                    ))}
                  </div>
                )}
              </div>
            </>
          ) : (
            <EmptyState
              isLoading={loading}
              condition={!outputCode}
              icon="fas fa-asterisk"
              title="Awaiting Pattern Rules"
              description="Describe your target text pattern matching requirements in natural English to synthesize a bulletproof regular expression."
              hint={<>Add multiple custom <code>Test Cases</code> below to watch real-time matching highlights trigger instantly.</>}
              loadingTitle="Crafting Regular Expression"
              loadingDescription="Compiling modifier boundaries, tokenizing pattern descriptions, and building capture groups..."
            />
          )}
        </div>
      </div>
    </div>
  );
}