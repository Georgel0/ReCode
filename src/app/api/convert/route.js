...
// Mock data is the heaviest output type; reserve the full budget for it.
const GROQ_MAX_TOKENS_DEFAULT = 8000;
const GROQ_MAX_TOKENS_MOCK = 24000;

// In turbo mode, cap row count so output JSON never blows the context window.
// 10 rows × 8 tables × ~5 columns ≈ ~6k tokens — safely within budget.
...
export async function POST(request) {
  try {
    ...
    const rawRowCount = Number(body.rowCount) || 15;
    const effectiveRowCount =
      type === 'mock' && qualityMode === 'turbo'
        ? Math.min(rawRowCount, TURBO_MAX_ROWS)
        : rawRowCount;
