...
export function useApiMocks() {
  ...
  const updateTestPayload = useCallback((idx, payload) => {
    setTestPayloads(prev => ({ ...prev, [idx]: payload }));
  }, []);

  const executeTestRequest = useCallback(async (handlerIdx) => {
    const handler = generatedData?.handlers?.[handlerIdx];
    if (!handler || !generatedData?.mockId) return;

    setIsTesting(prev => ({ ...prev, [handlerIdx]: true }));
    const startTime = Date.now();

    try {
      const baseUrl = `${window.location.origin}/m/${generatedData.mockId}`;

      // Auto-replace path parameters (like :id, {id}, [id]) with '123' so the test succeeds
      const resolvedPath = handler.path.replace(/[:{\[][a-zA-Z0-9_]+[}\]]?/g, '123');
      ...
      // Add body and content-type for mutations
      if (['POST', 'PUT', 'PATCH'].includes(method)) {
        options.headers['Content-Type'] = 'application/json';
        options.body = testPayloads[handlerIdx] || '{\n  \n}';
      }

      // If user has an error variant selected, force it via headers
      ...
      setTestResponses(prev => ({
        ...prev,
        [handlerIdx]: {
          error: err.message || 'Failed to connect to mock server',
          time: Date.now() - startTime
        }
      }));
    } finally {
      setIsTesting(prev => ({ ...prev, [handlerIdx]: false }));
    }
