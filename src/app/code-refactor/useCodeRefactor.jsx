...
import { convertCode, LANGUAGES, useDraft } from '@/lib';
...
export const suggestRefactorMode = (code) => {
  ...
  if (varCount > 0) {
    ...
    reasons.modern.push(`Replace ${plural(varCount, 'var declaration')} with const/let to prevent hoisting surprises`);
  }

  const thenCount = countMatches(clean, /\.then\s*\(/g);
  if (thenCount > 0) {
    scores.modern += thenCount * 2;
    reasons.modern.push(`${plural(thenCount, '.then() chain')} — flatten with async/await for clearer control flow`);
  }

  const requireCount = countMatches(clean, /\brequire\s*\(/g);
  if (requireCount > 0) {
    scores.modern += requireCount * 1.5;
    reasons.modern.push(`${plural(requireCount, 'require() call')} — migrate to ESM import/export`);
  }
  ...
  if (errCbCount > 0) {
    ...
    reasons.modern.push(`${plural(errCbCount, 'error-first callback')} — wrap in a promisify helper or rewrite with async/await`);
  }

  const looseEqCount = countMatches(clean, /==(?!=)|!=(?!=)/g);
  if (looseEqCount > 0) {
    scores.modern += looseEqCount;
    reasons.modern.push(`${plural(looseEqCount, 'loose equality check', 'loose equality checks')} (== / !=) — use === / !== to avoid type coercion bugs`);
  }
  ...
  if (nesting >= 2) {
    ...
    reasons.perf.push(`${nesting}-level nested loop → O(n^${nesting}) complexity — consider a Map/Set for O(n) lookup`);
  }

  const searchCount = countMatches(clean, /\.(indexOf|includes|find|findIndex)\s*\(/g);
  if (searchCount > 0) {
    scores.perf += searchCount * 0.4;
    reasons.perf.push(`${plural(searchCount, 'linear-search call', 'linear-search calls')} (.find/.includes etc.) inside loops — hoist to a Set/Map lookup`);
  }

  const cloneCount = countMatches(clean, /JSON\.parse\s*\(\s*JSON\.stringify/g);
  if (cloneCount > 0) {
    scores.perf += cloneCount * 2;
    reasons.perf.push(`${plural(cloneCount, 'JSON.parse(JSON.stringify()) clone')} — replace with structuredClone() or a targeted spread/slice`);
  }

  // Comments signals
  const commentLines = lines.filter((l) => /^\s*(\/\/|\/\*|\*)/.test(l)).length;
  const commentRatio = commentLines / nonBlankLines;
  const functionCount = countMatches(clean, /\bfunction\b/g) + countMatches(clean, /=>/g);

  if (nonBlankLines > 15 && commentRatio < 0.05) {
    scores.comments += 2;
    reasons.comments.push(`${Math.round(commentRatio * 100)}% comment coverage — add at least a one-liner above each exported function`);
  }

  if (functionCount > 2 && commentRatio < 0.1) {
    const bump = Math.min(functionCount * 0.5, 3);
    scores.comments += bump;
    reasons.comments.push(`${plural(functionCount, 'function', 'functions')} with <10% comment coverage — document parameters, return values, and edge cases`);
  }
  ...
  if (badNameCount > 0) {
    ...
    reasons.clean.push(`${plural(badNameCount, 'non-descriptive name', 'non-descriptive names')} (tmp, val, foo…) — rename to reflect the value's purpose`);
  }
  ...
  if (deepIndentLines > 0) {
    ...
    reasons.clean.push(`${plural(deepIndentLines, 'line', 'lines')} indented ≥4 levels — extract inner blocks into named functions`);
  }

  if (functionCount > 0 && nonBlankLines / functionCount > 40) {
    scores.clean += 2;
    reasons.clean.push(`Functions average ${Math.round(nonBlankLines / functionCount)} lines — split into smaller single-responsibility units`);
  }

  const magicNumCount = countMatches(noStrings, /[^\w.](?:[2-9]\d*|\d{2,})(?!\w)/g);
  if (magicNumCount > 0) {
    scores.clean += Math.min(magicNumCount * 0.25, 3);
    reasons.clean.push(`${plural(magicNumCount, 'magic number')} — extract into named constants (const MAX_RETRIES = 3)`);
  }
...
export function useCodeRefactor() {
  ...
  useDraft(
    ...
    },
    {
      ...
      skip: moduleData?.type === 'refactor' || isRestoring.current,
    }
  );
